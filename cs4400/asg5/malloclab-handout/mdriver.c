...
static int add_range(range_t** ranges, char* lo, int size, int tracenum, int opnum) {
  ...
  for (i = 0; i < size; i += page_size) {
    if (!pagemap_is_mapped(lo + i)) {
      sprintf(msg, "Payload (%p:%p) includes an unmapped page", lo, hi);
      malloc_error(tracenum, opnum, msg);
      return 0;
    }
  }
  if (!pagemap_is_mapped(lo + size - 1)) {
    sprintf(msg, "Payload (%p:%p) ends at an unmapped page", lo, hi);
    malloc_error(tracenum, opnum, msg);
    return 0;
  }
  ...
  for (i = 0; i < trace->num_ops; i++) {
    switch (trace->ops[i].type) {
      ...
      case REALLOC: /* mm_mealloc + mm_free */
        ...
        oldp = trace->blocks[index];
        if ((newp = mm_malloc(newsize)) == NULL) app_error("mm_realloc failed in eval_mm_util");
    ...
    if (heap_size > max_heap_size) max_heap_size = heap_size;
...
static void eval_mm_speed(void* ptr) {
  ...
  for (i = 0; i < trace->num_ops; i++) switch (trace->ops[i].type) {
      ...
      case REALLOC: /* mm_malloc + mm_free */
        ...
        newsize = trace->ops[i].size;
        oldp = trace->blocks[index];
        if ((newp = mm_malloc(newsize)) == NULL) app_error("mm_realloc error in eval_mm_speed");
        mm_free(oldp);
        trace->blocks[index] = newp;
        break;
  ...
  for (i = 0; i < trace->num_ops; i++) {
    switch (trace->ops[i].type) {
      ...
      case REALLOC: /* realloc */
        ...
        newsize = trace->ops[i].size;
        oldp = trace->blocks[index];
        if ((newp = malloc(newsize)) == NULL) unix_error("malloc failed in eval_libc_speed\n");
        free(oldp);
...
static void printresults(int n, stats_t* stats) {
  ...
  if (errors == 0) {
    printf("%12s%5.0f%%%5.0f%%%8.0f%10.6f%6.0f\n", "Total       ", (util / n) * 100.0,
           (inst_util / n) * 100.0, ops, secs, (ops / 1e3) / secs);
  } else {
