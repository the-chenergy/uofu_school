...
\begin{enumerate}
  ...
  \item (Graph DP)
    ...
    My proposed algorithm is as follows, whose first argument is an adjacency list representing the graph $G$.
    ...
    \begin{center}
      \begin{minipage}{\linewidth}
        ...
        \begin{algorithm}[H]
          ...
          \KwIn{\texttt{hash\_map<vertex, hash\_set<vertex>>} $A$, \texttt{int} $l$, \texttt{vertex} $s$, \texttt{vertex} $t$, \texttt{function(vertex, vertex):number} $w$}
          \KwOut{\texttt{number}}
          ...
          \texttt{hash\_map<vertex, number>} $M[0..l] \gets$ filled with $\{\}$

          \For{\texttt{int} $k = 0$ to $l$}{$M[k][t] \gets 0$}

          \For{each \texttt{vertex} $u$ in $A$'s keys}{$M[l][u] \gets -\infty$}

          \For{\texttt{int} $k = l-1$ to 0}
          {
          \For{\texttt{int} $i = n-1$ to 1}
          {
            ...
            \For{each \texttt{vertex} $v$ in $A[u]$}{$M[k][u] \gets \text{max}(M[k][u], w(u, v) + M[k+1][v])$}
            }
          }
