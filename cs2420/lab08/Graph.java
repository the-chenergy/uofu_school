...
/**
 * Represents a directed graph (a set of vertices and a set of edges). The graph is not generic
 * and assumes that a string name is stored at each vertex.
 * 
...
public class Graph
{
	 ...
	 * The edge is associated with the "weight". If either vertex does not already exist in the
	 * graph, it is added.
	 */
	...
	/**
	 * Uses Dijkstra's algorithm to determine the shortest paths from the starting vertex to every
	 * other vertex in the graph.
	 * 
	 * Use Java's PriorityQueue class to find the "unvisited vertex with smallest distance from s".
	 * In order to associate each vertex with a priority (related to its distance from s), you must
	 * either use the Comparable interface or the Comparator interface. See the API for
