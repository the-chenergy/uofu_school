...
namespace cs3505
{
  class string_set
  {
    ...
    //  between functions with object variables - use parameters and return
    //  values.)
  ...
  public:
    string_set(int max_next_width =
                   10); // Constructor.  Notice the default parameter value.
    ...
    bool contains(
        const std::string& target) const; // Const - does not change the object
    int get_size() const;                 // Const - does not change object

    string_set&
    operator=(const string_set& rhs); // Not const - modifies this object

    std::vector<std::string>
    get_elements() const; // Returns all the elements in this string_set, in
                          // ascending order.

  private:
    // private methods
    int get_next_width() const;
    void get_prev(std::vector<node*>& prev, const std::string& target) const;
    void clear();
  };
