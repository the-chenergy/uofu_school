...
namespace ChessBrowser {
	partial class Form1 {
		...
		private void UploadGamesToDatabase(string PGNFileName) {
			...
			// Read in the games and use that to estimate the total number of work steps.
			List<Game> games = Pgn.Read(PGNFileName).ToList();
			...
			using (MySqlConnection conn = new MySqlConnection(connection)) {
				try {
					...
					// Create and prepare the command used to upload a game.
					MySqlCommand command = CommandManager.CreateUploadGameCommand(conn);

					// Iterate through the games and execute appropriate insert commands.
					foreach (Game game in games) {
						CommandManager.ExecuteUploadGameCommand(command, game);

						// Work step completed; update the UI progress bar.
						WorkStepCompleted();
					}
				} catch (Exception e) {
			...
			using (MySqlConnection conn = new MySqlConnection(connection)) {
				try {
					...
					SearchGamesParams searchParams = new SearchGamesParams() {
						...
						Result = winner.Length > 0 ? winner[0] : '\0', // convert to W, B, D, or zero
						FilterDateRange = useDate,
						DateRangeStart = start.ToString("yyyy-MM-dd"),
						DateRangeEnd = end.ToString("yyyy-MM-dd"),
						ShowMoves = showMoves
					};

					// Generate and execute the search command, then parse the results into an
					// appropriate string.
					MySqlCommand command = CommandManager.CreateSearchGamesCommand(
						conn, searchParams);
					List<Game> results = CommandManager.ExecuteSearchGamesCommand(
						command, searchParams).ToList();

					numRows = results.Count;
					parsedResult = string.Join(
						"\r\n\r\n",
						from game in results
						select FormatSearchGamesResult(game, searchParams)
					);
				} catch (Exception e) {
					Console.WriteLine(e.Message);
				}
		...
		/// <summary>Formats a game returned from the search-game command.</summary>
		private string FormatSearchGamesResult(Game game, SearchGamesParams searchParams) {
			return string.Join(
				"\r\n",
				$"Event: {game.Event}",
				$"Site: {game.Site}",
				$"Date: {game.EventDate} 12:00:00 AM",
				$"White: {game.White} ({game.WhiteElo})",
				$"Black: {game.Black} ({game.BlackElo})",
				$"Result: {game.Result}",
				searchParams.ShowMoves ? $"Moves: {game.Moves}" : ""
			);
		}
