...
namespace LMS.Controllers {
	...
	public class AccountController : CommonController {
		...
		public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role) {
			string nextUId = GetNextUniqueUId();
			...
			try {
				...
				Debug.WriteLine("Error occurred while creating new user.");
				return "";
			}
		...
		private string GetNextUniqueUId() {
			var allUIds =
				from uIds in (from administrator in db.Administrators select administrator.UId)
									.Concat(from professor in db.Professors select professor.UId)
									.Concat(from student in db.Students select student.UId)
									.ToList()
				select uIds;
			string maxUId = allUIds.Max();
			string nextUId = "u" + (int.Parse(maxUId.Substring(1)) + 1).ToString().PadLeft(7, '0');
