    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult AllTitles()
        {
            ...
            using (Team120LibraryContext db = new Team120LibraryContext())
            {
                ...
                into checkedOutItems
                from checkedOut in checkedOutItems.DefaultIfEmpty()
                join patrons in db.Patrons on checkedOut.CardNum equals patrons.CardNum
                into patronsItems
                from patrons in patronsItems.DefaultIfEmpty()
                select new { isbn = title.Isbn, title = title.Title, author = title.Author, serial = (uint?)inventory.Serial, name = patrons.Name ?? "" };
        ...
        public ActionResult CheckOutBook(int serial)
        {
            using (Team120LibraryContext db = new Team120LibraryContext())
            {
                CheckedOut check = new CheckedOut
                {
                    CardNum = (uint)card,
                    Serial = (uint)serial
                };
                db.CheckedOut.Add(check);
        ...
        public ActionResult ReturnBook(int serial)
        {
            using (Team120LibraryContext db = new Team120LibraryContext())
            {
                ...
                where checkedOut.CardNum == card && checkedOut.Serial == serial
                select checkedOut;
